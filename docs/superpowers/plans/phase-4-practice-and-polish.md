...
describe("practiceStats", () => {
  ...
  it("orders the weakest cards by confidence, then by least recently seen", () => {
    ...
    expect(stats.weakest.map((entry) => entry.card.id)).toEqual([
      "f2",
      "f3",
      "f1",
    ]);
  ...
  it("caps weakest at five", () => {
    ...
    const attempts = many.map((item, index) => ({
      cardId: item.id,
      confidence: 1,
      seenAt: `2026-09-08T10:0${index}:00Z`,
    }));
  ...
  it("ignores an attempt for a card that no longer exists", () => {
    const stats = practiceStats(cards, [
      { cardId: "deleted", confidence: 1, seenAt: "2026-09-08T10:00:00Z" },
    ]);
  ...
  it("handles an empty deck", () => {
    expect(practiceStats([], [])).toMatchObject({
      total: 0,
      seen: 0,
      unseen: 0,
    });
...
export function latestByCard(attempts: Attempt[]): Map<string, Attempt> {
  ...
  for (const attempt of attempts) {
    ...
    if (!existing || new Date(attempt.seenAt) > new Date(existing.seenAt))
      latest.set(attempt.cardId, attempt);
  }
  return latest;
}
...
export function practiceStats(
  ...
  for (const attempt of latest.values()) {
    ...
    if (!lastSeenAt || new Date(attempt.seenAt) > new Date(lastSeenAt))
...
- [ ] **Step 5: Commit** — _present this message to the user; do not run it_
...
function client(
  order: string[],
  overrides: Partial<{ record: () => Promise<PracticeOrder> }> = {},
) {
...
async function session(
  order: string[],
  overrides?: Parameters<typeof client>[1],
  deck = cards,
) {
  const stub = client(order, overrides);
  const rendered = renderHook(() =>
    usePractice({ kitId: "k1", cards: deck, client: stub }),
  );
  await waitFor(() =>
...
describe("usePractice", () => {
  it("uses the order the server gave, not the deck order", async () => {
    ...
    expect(rendered.result.current.queue.map((item) => item.id)).toEqual([
      "f3",
      "f1",
      "f2",
    ]);
  ...
  it("reports an error state when the order cannot be fetched", async () => {
    ...
    const rendered = renderHook(() =>
      usePractice({ kitId: "k1", cards, client: stub as never }),
    );
...
export function usePractice({
  ...
  const [ratedThisSession, setRatedThisSession] = useState<
    Map<string, 1 | 2 | 3>
  >(new Map());
  ...
  const move = useCallback(
    (delta: number) => {
      setRevealed(false);
      setIndex((current) =>
        Math.max(0, Math.min(current + delta, queue.length)),
      );
    },
    [queue.length],
  );
  ...
  const rate = useCallback(
    async (confidence: 1 | 2 | 3) => {
      ...
      try {
        ...
        setCovered(result.covered);
        setNotCovered(result.notCovered);
        setRatedThisSession((current) =>
          new Map(current).set(card.id, confidence),
        );
        setRevealed(false);
...
export function ConfidenceButtons({
  ...
  return (
    <div
      ...
      role="group"
      aria-label="How confident did you feel?"
    >
...
export function SessionProgress({
  ...
  return (
    <div className="space-y-1.5">
      ...
      <p className="sr-only">
        {pluralise(deck - covered, "card")} not yet seen.
      </p>
    </div>
  );
}
    ...
    <dl className="flex flex-wrap gap-x-4 gap-y-1 text-xs text-slate-500">
      {HINTS.map((hint) => (
        <div key={hint.keys} className="flex items-center gap-1.5">
          <dt>
            <kbd className="rounded border border-slate-300 bg-white px-1.5 py-0.5 font-sans text-[11px] text-slate-700">
              {hint.keys}
            </kbd>
          </dt>
          <dd>{hint.action}</dd>
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="rounded-lg border border-slate-200 bg-white p-6">
        ...
        <p className="mt-2 max-w-prose text-lg font-medium leading-relaxed text-slate-900">
          {card.front}
        </p>
        ...
        <div className="mt-6 border-t border-slate-100 pt-6" aria-live="polite">
          {revealed ? (
            <>
              ...
              <p className="mt-2 max-w-prose leading-relaxed text-slate-800">
      ...
      {revealed && (
        <div className="space-y-2">
          ...
          <ConfidenceButtons
            ...
            disabled={saving}
            current={ratedThisSession.get(card.id)}
          />
        </div>
      )}
    ...
    <Card title="Session finished">
      <div className="space-y-5">
        <p className="max-w-prose text-sm text-slate-700">
          You rated {pluralise(ratedThisSession, "card")} this sitting. Across
          the whole deck you have seen {stats.seen} of {stats.total}
          {stats.unseen > 0 &&
            `, with ${pluralise(stats.unseen, "card")} still untouched`}
          .
        </p>
        ...
        <dl className="grid grid-cols-3 gap-3 text-sm">
          {(
            ...
            <div key={label} className="rounded-md border border-slate-200 p-3">
              ...
              <dd
                className={`mt-0.5 text-lg font-semibold tabular-nums ${tone}`}
              >
                {value}
              </dd>
            </div>
        ...
        {stats.weakest.length > 0 && (
          <div>
            ...
            <p className="mt-0.5 text-xs text-slate-500">
              Ordered by lowest confidence, then by longest since you last saw
              them — which is exactly the order the next session will use.
            </p>
...
function Session({ doc }: { doc: KitDoc }) {
  ...
  if (session.status === "loading")
    return <StateBlock state="loading" title="Setting up your session" />;
  if (session.status === "error") {
    return (
      <StateBlock
        ...
        action={
          <Button onClick={() => void session.restart()}>Try again</Button>
        }
      />
    );
  }
...
- [ ] **Step 8: Commit** — _present this message to the user; do not run it_
...
if (
  ...
  if (typeof window !== "undefined")
    window.dispatchEvent(new Event("ipk:signed-out"));
}
```
...
- [ ] **Step 7: Commit** — _present this message to the user; do not run it_
